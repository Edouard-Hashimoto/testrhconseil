...
const uploadImage = async (file) => {
  ...
  try {
    ...
    const res = await $fetch('/api/upload-equipe-image', { method: 'POST', body: form })
    return res.filename
  } finally {
...
const startEdit = (membre) => {
  editingId.value = membre.id
  editData.value = { ...membre }
}
...
const saveEdit = async (evt) => {
  ...
  try {
    ...
    await $fetch(`/api/equipe/${editingId.value}`, { method: 'PUT', body: { ...editData.value, image } })
  ...
  <div class="admin-layout">
    ...
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        <NuxtLink to="/admin/equipe" class="nav-item active">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" /></svg>
          Équipe
        </NuxtLink>
    ...
    <div class="main-content">
      <header class="page-header">
        <div>
          ...
          <p class="page-desc">Gérez les membres de l'équipe</p>
        </div>
        <div class="stat-pill">
          <span class="stat-value">{{ equipe?.length ?? 0 }}</span>
      ...
      <section class="card">
        ...
        <form @submit.prevent="createMembre" class="form-row">
          <div class="field grow basis-full">
            <label>Nom</label>
            <input v-model="newMembre.nom" type="text" placeholder="ex: Fanny BOREL" required />
          </div>
          <div class="field grow basis-full">
      ...
      <section class="card">
        ...
        <div v-if="!equipe || equipe.length === 0" class="empty-state">
          Aucune personne enregistrée. Utilisez le formulaire ci-dessus pour en ajouter une.
        </div>
          ...
          <table class="services-table">
            ...
            <tbody>
              <tr v-for="membre in equipe" :key="membre.id">
                <template v-if="editingId === membre.id">
                  <td>
                ...
                <template v-else>
                  ...
                  <td class="text-right">
                    <button @click="startEdit(membre)" class="btn-edit">Modifier</button>
                    <button @click="deleteMembre(membre.id)" class="btn-del">Supprimer</button>
                  </td>
                </template>
              </tr>
            </tbody>
