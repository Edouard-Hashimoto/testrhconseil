...
const uploadParticulierImage_post = defineEventHandler(async (event) => {
  ...
  const filename = `particulier_${Date.now()}.${ext}`;
  const buffer = Buffer.from(await file.arrayBuffer());
  await writeFile(join(dir, filename), buffer);
  return { filename };
});

export { uploadParticulierImage_post as default };
//# sourceMappingURL=upload-particulier-image.post.mjs.map
